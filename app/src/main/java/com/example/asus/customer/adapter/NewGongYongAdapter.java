...
public class NewGongYongAdapter extends RecyclerView.Adapter<NewGongYongAdapter.ViewHolder> {

    private Context context;
    public ListDataCallBack listDataCallBack;
    private List<NewQiYeBean.BodyBean> list;
    ...
    public NewGongYongAdapter(Context context, List<NewQiYeBean.BodyBean> list) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, final int i) {
        if (list.size() > 0 && list != null) {
            NewQiYeBean.BodyBean bean = list.get(i);
            String name = bean.getIndustryTypeName();
